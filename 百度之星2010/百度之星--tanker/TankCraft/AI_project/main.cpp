...
int main(int argc,char *argv[])
{
	...
	while(1)
	{
		...
		recv_len=recv(sockConn,recvBuf,sizeof(DataForAI)+1,0);
		...
		if (strcmp(recvBuf,"Initiate_red")==0)
		{
			*(InitiateInfo *)sendBuf = chooseType();
			...
			else
			{
                ...
                {
                    Sleep(1);
                    recv_len+=recv(sockConn,recvBuf+recv_len,sizeof(DataForAI)+1-recv_len,0);
                }
				*(Order *)sendBuf = makeOrder(*(DataForAI *)recvBuf);
            }
		send(sockConn,(const char *)(sendBuf),128,0);
